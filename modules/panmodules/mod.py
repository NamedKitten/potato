...
class Moderation(commands.Cog):
    ...
    async def hackban(self, ctx, user_id: int):
        ...
        """
        try:
            await self.potato.http.ban(str(user_id), str(ctx.guild.id))
            await ctx.send('Done. Good riddance.')
    ...
    async def kick(self, ctx, member: discord.Member, reason: str=None):
        ...
        * reason: The reason for kicking this user
        ...
        try:
            ...
            self.potato.dispatch('kick', member)  # yay for implementing on_kick
            await ctx.send('Done. Good riddance.')
        except discord.Forbidden:
...
def setup(potato):
    potato.setup_module(Moderation(potato))
